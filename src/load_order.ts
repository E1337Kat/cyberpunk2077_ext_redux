...
export const pruneToSparseLoadOrder = async (
  ...
  const newLoadOrderEntries =
    pipe(
      ...
      /*
      async (entries) => {
        vortexApi.log(`debug`, `${me}: Load order for redMod.exe after pruning: ${(await entries).map((entry) => entry.redmodName)}`);
        return entries;
      },
      */
    );
...
const deployAndSerializeNewLoadOrder: VortexWrappedSerializeFunc = async (
  ...
  vortexApi.events.once(`did-deploy`, async () => {
    ...
    const yay = await maybeREDdeployed();

    if (isLeft(yay)) {
      // boo
      vortexApi.log(`error`, `${me}: REDmod deployment ${loID} failed! ${S(yay.left)}`);
