...
import {
  testForFallback,
  installFallback,
  fallbackToPromptOrFailOnUnaccountedForFiles,
...
import { dot } from "./installers.utils";
...
export const wrapInstall =
  ...
  async (
    ...
    const finalInstructions = stillMissingSourceFiles
      ? await fallbackToPromptOrFailOnUnaccountedForFiles(
          vortexApi,
          installer.type,
          treeForInstallers.fileTree,
          allInstructionsWeKnowHowToGenerate.map(dot(`source`)),
        )
