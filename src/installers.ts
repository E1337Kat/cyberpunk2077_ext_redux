...
import { KNOWN_TOPLEVEL_DIRS } from "./installers.layouts";
import {
  testForCetCore,
  installCetCore,
...
export const wrapInstall =
  ...
  async (
    ...
    let { instructions } = await installer.install(
      vortexApi,
      vortexLog,
    ...
    if (instructions.length < fileCount(treeForInstallers.fileTree)) {
      // Might need to add some types for *intentional* fewer or more
      // files in the future but for now let's try this.
      //
      // We can also inject generic text file, image etc. copy here
      // Defect: https://github.com/E1337Kat/cyberpunk2077_ext_redux/issues/133
      //
      vortexApi.log(
        `error`,
        `The instruction count doesn't handle all source files. Reverting to Fallback Installer!`,
      );

      const fallbackInstructions = await fallbackInstaller.install(
        vortexApi,
        vortexLog,
        sourcePaths(treeForInstallers.fileTree),
        treeForInstallers.fileTree,
